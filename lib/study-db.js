...
class StudyDB {
    ...
    async updateDeckSnapshot(deckId) {
        const cards = await this.getCardsWithProgress(deckId);
        let counts = { 0: 0, 1: 0, 2: 0, 3: 0, 4: 0 };
        cards.forEach(c => { counts[c.progress?.level || 0]++; });
        ...
        return new Promise((resolve, reject) => {
            ...
            getReq.onsuccess = () => {
                ...
                if (!deck.snapshots) deck.snapshots = [];
                const now = Date.now();
                const lastSnapshot = deck.snapshots[deck.snapshots.length - 1];
                const newSnapshot = { timestamp: now, counts };

                if (lastSnapshot && (now - lastSnapshot.timestamp) < 300000) {
                    deck.snapshots[deck.snapshots.length - 1] = newSnapshot;
                } else {
                    deck.snapshots.push(newSnapshot);
                }
